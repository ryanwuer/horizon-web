...
export default (props: any) => {
  ...
  const editing = pathname.endsWith('editv2/vm');
  const creating = pathname.endsWith('newinstancev2/vm');
  ...
  const pageOrders = useMemo(() => (editing ? [0, 2, 3] : [0, 1, 2, 3]), [editing]); // Remove build config step
  ...
  useRequest(() => getApplicationV2(id), {
    ready: creating,
    onSuccess: (data) => {
      const { name: appName, tags: appTags } = data;
      setApplicationName(appName);
      if (!copying) {
        // basicInfo - for VM deployment, only environment and tags
        form.setFields([
          ...
          <StepContent>
            {
              pageOrders[current] === 0 && (
                <BaseInfoForm
                  ...
                  clusterType={AppOrClusterType.VM}
            ...
            }
            {
              ...
              pageOrders[current] === 1 && (
                <TemplateForm
                  ...
                  type={CatalogType.VirtualMachine}
                />
              )
            }
            {
